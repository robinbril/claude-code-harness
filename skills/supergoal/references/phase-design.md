...
- **8–12 phases**: large features, full-stack new-project apps, complex refactors that need a safety-net + staged migration.
- **13+ phases**: major migrations, multi-system rewrites, existing-codebase untangles. Don't combine just to hit a target number; combining hides dependencies.
...
### Existing-codebase refactor shape
...
## Cleanliness (grep-checked by the evaluator)

The evaluator reports three grep-based counts against the complete set of added/new lines since the baseline, `bash .supergoal/repo-state.sh added-lines <Baseline ref>` (includes uncommitted + untracked work, not just committed; see `repo-state-comparison.md`):
...
The override is read by the evaluator and counts are reported but not failed. Keep overrides narrow: they're a release valve, not a default.
...
For each phase, list what the generator must surface into the conversation and what the evaluator will re-run or observe. Behavior-producing phases must include at least one `empirical` item: the running artifact observed, not just "tests pass". Common evidence types:
